...
**对象**：编辑态底栏「移入分类」→ 底部抽屉（标题行 + 分类勾选列表 + 完成按钮）。**2026-09-18 追加**：面板最顶端新增常驻搜索框、顶端留白与左右留白对齐（成同心观感）、面板高度随之 62% → 75%（宿主已于 2026-09-17 抽为共用组件 `components/FavCategoryPicker.ets`，详见 §12 变更记录）。
...
| `.height('62%')` | `height: '62%'` → **`'75%'`**（2026-09-18 两轮：67% → 75%） | ⚠️ 用 **`Length` 字符串**而非 `SheetSize`：枚举只有 MEDIUM(50%) / LARGE(90%) / FIT_CONTENT，要保住非标准档位只能给百分比。2026-09-18 因面板最顶端新增搜索框 62% → 67%，同日用户真机看图后再拍板加大 → **75%**（新增内容净高 ≈ 52vp = 框 40 + 与标题行间距 `Spacing.xs` 4 + 顶端留白 8 → 16，远小于档位放大值 → **列表可视区不降反增**）；仍走同一 `Length` 字符串写法（§8 坑 18 口径不变） |
...
| 2026-09-18 | **追加·收藏归类面板（`components/FavCategoryPicker.ets`）新增常驻搜索框 → 顶端留白对齐左右 + 面板高度 `62% → 75%` + 卡片圆角 `16 → 20`**（同日三轮：留白与高度为用户真机看图后拍板；圆角为用户要求「增大并给出可微调参数」）（用户要求：自定义分类多起来后能靠搜索快速找到分类再移入，搜索框放「面板最顶端」）。**内容侧**：面板最顶端插入胶囊搜索框（`sys.symbol.magnifyingglass` + `TextInput`，高 40、宽 `calc(100% - 32vp)`，与标题行 / 列表同取 `Spacing.lg` 左右外边距 → 三者边缘对齐）；材质与列表条目**同源** `ImmMaterial.cardAction()` + 半透明底 `#B3FFFFFF` / `#B31B1E24`（**不用不透明底**，守 §8 坑 6）；新增 `@State query` + `filteredRows()`（空串原样返回 → 「未分类恒首位 + 分类顺序」零变化；有词按名称大小写不敏感子串匹配）+ `noMatch()` 空态（列表内插一条 160 高「未找到相关分类」提示，**不替掉整个 List** → 下方常驻「新建分类」入口不被挤掉）；`resetCreateState()` 更名 `resetInputs()`，关闭面板一并回收搜索词；**刻意不设 `defaultFocus`**（打开面板默认不起键盘，列表浏览不被遮挡）。**几何侧**（同日两轮）：① 外层 Column 顶端留白 `8 → Spacing.lg`(16)，与搜索框 / 标题行 / 列表同取左右 `Spacing.lg` → **上 / 左 / 右三边等距**，胶囊圆角（full，40 高 → r20）嵌在面板圆角（r24）内成同心观感（原 8 / 16 视觉上「上窄旁宽」）；② `height: '62%' → '67%' → '75%'`（竖向 `Length` 字符串写法不变）—— 新增内容净高 ≈ 52vp（框 40 + 与标题行间距 `Spacing.xs` 4 + 顶端留白 +8），远小于档位放大值 → **列表可视区不降反增**，遮罩只剩顶部一条；③ **面板卡片圆角 `Radius.lg`(16) → 文件级常量 `PICKER_CARD_RADIUS = 20`**（三处卡片：分类条目行 / 新建分类输入行 / 「＋ 新建分类」入口行，**单点微调** —— 只改这一个数即全同步；**不引全局 `Radius.lg`** 是刻意的：面板卡片要比全局卡片更饱满且需随时回调，全局 token 不动以免波及其它页卡片）；取 20 的依据 = 与面板顶部搜索胶囊同值（40 高全圆角 → r20）、与面板自身顶圆角（系统半模态 24）成 24 / 20 / 20 递减梯度，卡高 76（内容 44 + padding 16×2）→ 半径上限 38 才变胶囊，20 仍在圆角矩形区间，可调档位 16 / 20 / 24。**遗留（挂真机门）**：面板 **75%** + 键盘 ≈38% 已远超整窗，系统顶起 / 压缩 sheet 时**最顶端的搜索框**可能被顶出屏；回退 = 搜索框挪到标题行之下 + 高度回调 62%，或高度退回 67%（已写入代码注释）。§6.8 对象行与几何表 `height` 行同步。构建 `BUILD SUCCESSFUL`；lint 0；待真机复验 |
