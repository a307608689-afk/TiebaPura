...
> 更新：2026-09-17 · **列数档位回归「按窗口横竖屏」——竖 2 / 横 3（宽度分档作废，构建通过）**：用户复看小窗三档后把上一轮的**按窗口宽度分档**（`≥840→3 / 600~840→2 / <600→手机档`）整体回退为**按窗口横竖屏**：`isLandscape ? 3 : 2`（**竖屏一律 2 列**，「≥840 竖屏 3 列」作废；`<600` 窄窗仍在首行闸门回手机档）。改动 **9 文件 10 处列数函数**：① 8 个列数页（HomeTab〔`updateColumns` 按 display 横竖屏把 3/2 写入 `this.columns`，`homeColumns()` 只加闸门不再吃 `Breakpoint.md`〕/ ForumsTab / MessagesTab / Favorite / PersonalContent / Search / ThreadList / UserProfile）；② 楼中楼 SubPostDetail `replyColumns`（由二次拍板的「≥840 恒 3」同口径改 `isLandscape() ? 3 : 2`）。**设备形态分支与手机闸门零改动**（阔直屏竖 1 横 3 / 大折叠竖 2 横 3 / Pura X Max / `<600` 手机档仍在各自分支处先行返回）；**ThreadDetail 回复区维持 09-16「平板横竖屏均 2 列」拍板**，不受本档影响（其 `replyColumns` 三次口径下仍是 `return 2`）。`tools/build.ps1` → **BUILD SUCCESSFUL**；**lint 0**；**待真机复验**（平板竖屏 2 列、横屏 3 列、旋转实时切换、自由多窗随宽高比翻档〔横扁小窗→3 / 竖高小窗→2〕、手机档逐像素不变）。
> 更新：2026-09-17 · **§4.9 宫格单格高收口（大折叠 / Pura X Max 防拉伸补全，构建通过）**：`gridCellHeight()`（大折叠 / Pura X Max 展开 = 72、其它形态 = 120）此前只用在 `Grid` **总高**表达式上，而 `GridItem` 内的 `Stack` 与 `ThreadGridImage` 仍硬编码 `.height(120)` → 那两类形态「总高按 72 行算、格子按 120 渲染」自相矛盾，72 档的防拉伸实际未生效（表现 = 图被撑高 / 行高被顶开）。改为**一处 `gridCellHeight()` 三处同源**：内层 `Stack`、`ThreadGridImage`、`Grid` 总高，外加楼层卡估算器 `imgRows × 格高` 项与两处注释。**手机档 / 平板 / 阔直屏 = 120 逐像素不变**；阔直屏回复单列不在 72 档、不受影响。`tools/build.ps1` → **BUILD SUCCESSFUL**（34.5s）；**lint 0**；**待真机复验**（大折叠展开态、Pura X Max 展开态宫格比例，手机 / 平板回归）。
...
/** 回复区列数：手机 / 窄窗恒 1；阔直屏恒 1；大折叠 2；平板**横竖屏均 2**（09-16 拍板，三次拍板未覆盖） */
private replyColumns(): number {
  if (this.currentWidth > 0 && this.currentWidth < Breakpoint.sm) return 1;  // 折叠态 / 分屏 / 自由多窗窄窗
  if (isWideBarDevice()) return 1;    // 阔直屏
  if (isLargeFoldDevice()) return 2;  // 大折叠
  if (!this.isWideFormDevice) return 1;
  if (this.currentWidth < Breakpoint.sm) return 1;
  return 2;   // 平板横竖屏均 2（09-16 拍板，独立于三次拍板的竖 2 / 横 3）
}
...
/** 回复区列数：手机 / 窄窗 / 阔直屏 / 大折叠恒 1；平板**竖 2 / 横 3**（09-17 三次拍板） */
private replyColumns(): number {
  if (this.currentWidth > 0 && this.currentWidth < Breakpoint.sm) return 1;  // 窄窗闸门最前
  if (isWideBarDevice()) return 1;    // 阔直屏
  if (isLargeFoldDevice()) return 1;  // 大折叠
  if (!this.isWideFormDevice) return 1;
  if (this.currentWidth < Breakpoint.sm) return 1;
  return this.isLandscape() ? 3 : 2;  // 平板竖 2 / 横（分栏后右栏内）3
}
...
### 4.13 「列数调整」全局档位（个性化页 + 8 个多列页，2026-09-18 已落地）

- **适配需求**：大屏形态下用户希望**显式**控制列表列数，而非只能吃设备自动档。档位固定三档、文案固定「默认 / 竖二横三 / 竖三横四」；切档后各列表页**强制回到列表顶部**（用户拍板「强制」，明确不接受保留页内滚动位）。
- **设计方案（用户拍板「按推荐方案来」）**：
  - **状态真源单一**：AppStorage `columnMode`（`0` / `1` / `2`）+ `CACHE_KEY.COLUMN_MODE`（`'tieba_column_mode'`）持久化。**唯一写入口** = `UsageHabitsManager.setColumnMode(mode)`（写 AppStorage → bump `columnModeTick` → 落盘）；个性化页只读显示（`@StorageLink('columnMode')`），不自行写值。
  - **策略层单点**（`common/Theme.ets` 新增 4 个导出函数）：`readColumnMode()`（读 AppStorage，非法归 0）/ `columnModeLabel(mode)`（三档文案）/ `isLargeScreenForm(isWideForm)`（`isWideForm || isWideBarDevice() || isLargeFoldDevice()`）/ `overrideColumns(mode, isLandscape)`。
  - **档位语义**：`0` → `overrideColumns` 返回 **`0` = 不覆盖**（各页原自动适配分支逐像素不变）；`1` → 横 `3` / 竖 `2`；`2` → 横 `4` / 竖 `3`。**不做**最小卡宽护栏（用户否决方案 B）。
  - **消费层统一块**：8 页列数函数在**窗口闸门（`pageWidth < 600`）之后**插入，保持「设备恒值分支在前、闸门在更前」的既有次序不被破坏（§6.1 D4）：
    ```
    ...
    ```
  - **档位广播兜底（`fontTick` 同范式）**：各页 `@StorageLink('columnModeTick') @Watch('onColumnModeTick')` + 本地 `@State columnMode`——离屏缓存页的 `@StorageLink` 同步不触发重绘，故靠 tick 显式回写本地态。
  - **冷启动首帧**：`UsageHabitsManager.init()` 的 bump 可能早于列表页构造 → `@Watch` 不会补触发，故 **8 页 `aboutToAppear` 各自直读** `this.columnMode = readColumnMode()`（同一真源、不产生第二份状态）。
  - **强制回顶**：`onColumnModeTick()` 内对各页主滚动容器 `scrollTo({ xOffset: 0, yOffset: 0 })`（包 `try/catch` 静默——容器未 attach / 不在当前分支时忽略）。`ForumsTab` / `PersonalContent` 原无 scroller，**为此新增 `Scroller` 字段**并挂到主 `List`。
- **涉及改动点（13 文件）**：
  1. `common/Constants.ets`：`CACHE_KEY.COLUMN_MODE`（值 `'tieba_column_mode'`）。
  2. `service/UsageHabitsManager.ets`：`init()` 读档（`1/2` 以外一律归 `0`；catch 分支同样写 `0`）、`getColumnMode()`、`setColumnMode(mode)`、`persistColumnMode(mode)`。
  3. `common/Theme.ets`：`readColumnMode` / `columnModeLabel` / `isLargeScreenForm` / `overrideColumns`。
  4. 8 个多列页（`HomeTab` / `ForumsTab` / `Favorite` / `MessagesTab` / `PersonalContent` / `UserProfile` / `ThreadList` / `Search`）：import + `columnModeTick` 链接 + `columnMode` 本地态 + 列数函数覆盖块 + `onColumnModeTick()`（回写 + 强制回顶）+ `aboutToAppear` 直读；`ForumsTab` / `PersonalContent` 追加 `Scroller` 字段与 `List({ scroller })` 挂点。
  5. `pages/PersonalizedPage.ets`：新增「列数调整」设置行（左 `sys.symbol.grid` + 标题 + 副标题「平板 / 折叠屏展开的列表列数」+ 右侧 `columnModeLabel` 显示值 + `chevron_right`），点击开 `ColumnModeDialog`（`@CustomDialog` 三选一：选中项 `sys.symbol.checkmark_circle` + 品牌蓝底，取消钮同款玻璃底）；弹窗 `width = dialogCardWidth()`（**数值 vp** = `max(200, min(400, 屏宽 − 48))`，与收藏页 / 进吧页 / 吧主页 CustomDialog 同款。⚠️ 不可用 `overlayCardWidth()`——`CustomDialogControllerOptions.width` 类型是 `Dimension`，**不接受** `'calc(100% - 48vp)'` 字符串，2026-09-18 首轮构建即报 `Type 'string | number' is not assignable to type 'Dimension'`）、`offset.dy = -110`（与全站置顶弹窗同一纵向位置）、`options.systemMaterial = ImmMaterial.dialog()`。
- **断点 / 阈值**：
  - 生效闸门 = `isLargeScreenForm(isWideFormDevice || isWideBarDevice() || isLargeFoldDevice())`（**复用既有形态函数，不新增第三套口径**）。
  - 不生效路径 = ① 窄窗闸门 `pageWidth > 0 && pageWidth < 600`（在覆盖块**之前**返回手机档）；② 手机形态；③ 默认档（`overrideColumns` 返回 `0`）。
  - 档位值 = `1 → 横 3 / 竖 2`；`2 → 横 4 / 竖 3`。
- **风险与回归项**：
  1. **默认档零回归**（`ov = 0` 落回原分支）是本方案的核心护栏：平板全屏竖 2 / 横 3、阔直屏竖 1 横 3、大折叠竖 2 横 3、Pura X Max 展开等既有取值在默认档下**逐像素不变**。
  2. **手机 / 窄窗不覆盖**：手机形态与窄窗（`<600`）在覆盖块之前已 return，档位对手机**完全无感**；折叠屏折叠态非 EXPANDED 时 `isLargeFoldDevice()` / `isWideBarDevice()` 均为 false → 同手机档。
  3. **强制回顶的取舍**：用户明确要求，切档即回顶 → **不保留页内滚动位置**（与本工程「回 Tab 保留位置」的既有偏好相反，属用户显式拍板项）。
  4. **列数消费方一律走列数函数**（09-17 教训）：本次只改 `xxxColumns()` 返回值，列模板 / 骨架 / 行分组 / `cachedCount` 经函数自动跟随，无「直绑中间值」隐患。
  5. **未纳入范围**：`ThreadDetail` / `SubPostDetail` 回复区（`replyColumns`）不在本次三档内，口径仍为既有恒值（平板恒 2 / 阔直屏恒 1 / 大折叠 2）；若需纳入另开一节。
- **风险自检（§6.2，强制）**：A 错位 → **无**（覆盖块位于窗口闸门之后、各页原分支之前，列数单点出口未变；块内只 `return` 一个整数，不引入新几何）。B 出屏 → **无**（列宽仍 `1fr` / `layoutWeight(1)`，只换列数；`4` 列档只可能在横屏宽窗命中，平板横屏 ~1280 → 列宽 ≈ 300vp 充裕）。C 重叠 → **无**（容器 / 层级 / 悬浮层 / 让位零改动）。D 手机回归 → **无**（`isLargeScreenForm` 手机恒 false、窄窗闸门在覆盖块之前，两条路径都不进覆盖块；`overrideColumns` 默认档返回 0 使平板也回原分支；必测 = 手机全屏竖 / 横逐像素、平板三档逐档、阔直屏与大折叠展开态三档、自由小窗（`<600`）不覆盖、折叠屏展开↔折叠、重启后档位保持）
- **追加（2026-09-18 用户反馈「对阔直屏不生效」→ 单加白名单）**：
  - **症状**：Pura X View（型号代码 **VOL-AL00**，2232×1320，16:9.5 阔型屏）在「列数调整」里切档**竖屏无任何变化**（横屏有效）。
  - **根因**：`isLargeScreenForm()` 本就含 `isWideBarDevice()`（型号白名单 `WIDE_BAR_MODEL = 'VOL-AL00'` + 物理比例带 (1.4,1.85)），所以覆盖块的第二个条件**本来就成立**；真正的拦路者是 8 页列数函数**首行的窄窗闸门** `pageWidth > 0 && pageWidth < Breakpoint.sm → return 手机档`——阔直屏**竖屏全屏**的窗口宽只有约 **440vp**（屏短边 1320px ÷ density），远小于 600，被闸门当成自由小窗直接回落单列，根本走不到覆盖块（横屏窗口宽 ≈744vp ≥ 600 故"横屏生效、竖屏不生效"）。
  - **方案**：新增 `Theme.isWideBarFullscreen(pageWidth)` 作为阔直屏竖屏的**唯一放行口**。判据 = ① 阔直屏档设备（`isWideBarDevice()`：**型号白名单 VOL-AL00** / 物理比例带 (1.4,1.85)，同档含 Pura X 展开态与 Pura X Max 展开）**且** ② 窗口宽 ≈ 屏幕短边 vp（`Math.min(d.width, d.height) / d.densityPixels`，容差 24vp，即窗口占满屏宽 = 全屏）。自由小窗与左右分屏的窗口宽必然小于屏短边 → 仍返回 false，**2026-09-17「小窗 <600 一律回落 1 列」的结论不被破坏**；未就绪 / 异常一律 false（安全方向）。
  - **改法**：8 页闸门统一改为 `if (this.pageWidth > 0 && this.pageWidth < Breakpoint.sm && !isWideBarFullscreen(this.pageWidth))`（ForumsTab 那处字面量 `600` 同款），各页补 `isWideBarFullscreen` 导入；个性化页设置行副标题同步改为「平板 / 阔直屏 / 折叠屏展开的列表列数」。
  - **零回归论证**：默认档（`overrideColumns` 返回 0）放行后继续走下面的设备恒值分支——阔直屏竖屏落到 `if (isWideBarDevice()) return this.columns / this.isLandscape ? 3 : 1`，而 `updateColumns()` 写的正是竖 1 → 与改前「闸门拦回 1 列」**同值、逐像素不变**；手机档不受影响（本函数首行 `isWideBarDevice()` 对手机恒 false，且手机侧仍被闸门 `<600` 拦住）；平板 / 大折叠竖屏窗口宽 ≥600，本就不进闸门。
...
| 2026-09-17 | **列数档位回归「按窗口横竖屏」竖 2 / 横 3（宽度分档作废）**：9 文件 10 处列数函数末档由「按窗口宽度分档 `≥840→3 / 600~840→2`」回退为 `isLandscape ? 3 : 2`（竖屏一律 2 列）；楼中楼 SubPostDetail `replyColumns` 同口径（其余设备分支 / 手机闸门 / ThreadDetail 恒 2 不动）。`tools/build.ps1` → **BUILD SUCCESSFUL**；**lint 0**；**待真机复验**（竖 2 / 横 3、旋转实时切换、小窗随宽高比翻档） | A 错位：**无**（列宽仍全 `1fr` / layoutWeight，只换列数取值来源）。B 出屏：**无**（竖屏列数 3→2 只会更宽松）。C 重叠：**无**（容器 / 层级 / 悬浮层零改动）。D 手机回归：**无**（手机全屏仍在首行 `<600` / `!isWideFormDevice` 闸门回手机档〔ForumsTab 2 列〕，本档不在手机路径；阔直屏 / 大折叠 / PuraXMax 恒值分支在闸门前返回、取值未动；必测 = 手机全屏竖/横逐像素、平板竖 2 横 3、旋转翻档、小窗横扁→3 / 竖高→2、折叠屏展开↔折叠） |
| 2026-09-17 | **分类变更跨页回流（§6.8 数据链路修复，纯逻辑 / 几何零改动）**：新增 AppStorage token `favCatChanged`，写方 = `FavCategoryPicker.commitCreate()`（面板内新建分类）+ `ThreadDetail.applyPickCategory()`（完成归类）；读方 = 收藏页 `@StorageLink @Watch` → 重读 `loadCategories` + `loadMappings`（本地零网络）。不动任何容器 / 列数 / 间距 / 材质，仅新增一条广播订阅。`tools/build.ps1` → **BUILD SUCCESSFUL**；**lint 0**；**待真机复验** | A 错位：**无**（零几何改动）。B 出屏：**无**。C 重叠：**无**。D 手机回归：**无**（收藏页仅在收到广播时整体替换 `categories` 数组，不触碰滚动位与选中态；已加"当前分类被外部删除 → 回落分类根视图"防御；必测 = 详情页新建分类后切回收藏页（无需重启即可见）、收藏页自身新建 / 重命名 / 删除 / 排序仍即时、面板移入归类后「已含 N/M」计数正确） |
| 2026-09-17 | **收藏归类面板「新建分类」行加号列对齐（面板内几何微调，D 类零回归）**：收起态第一列由「20 透明占位」改为「20 宽列内居中加号」→ 加号与上方勾选圆同列对齐、文字随之前移一列、删掉第二列 44 图标圆；加号列高 44 保证行高（16×2 + 44）与分类行一致不塌行；展开态与其余条目零改动。`tools/build.ps1` → **BUILD SUCCESSFUL**；**lint 0**；**待真机复验** | A 错位：**无**（行内无网格 / 通栏语义，仅单个入口行换列；行高靠 44 列高固定）。B 出屏：**无**（20 窄列，宽度不增）。C 重叠：**无**（加号去掉底色圆，与「已勾选」实心圆不再有视觉混淆）。D 手机回归：**无**（组件内改动，两宿主自动同款；不改列数 / 外边距总量 / 面板高度；必测 = 收藏页移入面板与详情页长按面板两处的新建分类行、展开输入行、折叠屏展开↔折叠） |
| 2026-09-17 | **收藏归类面板抽为共用组件（`components/FavCategoryPicker.ets`）+ 详情页长按收藏钮入口 + 新建分类重名拦截**：① 收藏页面板 UI 与勾选 / 新建逻辑整体迁入组件，根节点 `.bindSheet` 与 `MovePanelContent` 移除，落库改由宿主 `applyMove(checked)` 承接（几何 / 材质 / 文案零改动）；② 详情页收藏钮短按不变、长按 350ms 开同一面板，`applyPickCategory` 仅在「完成」里先补收藏再替换式归类，下滑 / 点遮罩关闭零副作用；③ 面板内新建、分类管理新建 / 改名、重命名弹窗三处统一重名拦截（改名排除自身）。`tools/build.ps1` → **BUILD SUCCESSFUL**；**待真机验收** | A 错位：**无**（面板内容与几何逐项未变；唯一变化是挂载节点由页面根 Stack 改为组件自身 0×0 根节点，`bindSheet` 为屏幕锚定模态、与挂载节点尺寸无关）。B 出屏：**无**（面板宽高 / 圆角 / 遮罩未变；组件根 0×0 不参与宿主布局）。C 重叠：**无**（面板走系统半模态独立层级，宿主页面无新增浮层）。D 手机回归：**无**（不引入形态 / 列数分支；收藏页移入面板入口、几何、文案逐像素不变；详情页仅新增长按手势、短按 `onClick` 行为不变；必测 = 收藏页移入面板开合与勾选、详情页长按弹面板 / 短按收藏、三处重名提示、折叠屏展开↔折叠） |
